...
import { ScrollView,View, Text,Image,StyleSheet, TouchableHighlight,SectionList ,ActivityIndicator} from 'react-native';
...
async function init(){
    ...
    let link = $(titular).find('a').attr('href');
...
_isMounted = false;//para evitar que se cargue mientras no esta montado
componentDidMount(){
...
componentWillUnmount() {
  ...
  render() {
    return (
        ...
        {this.state.loading ? 
            <CardNews
            datosTitular={this.state.titular}
            datosNews = {this.state.news}
           /> :
